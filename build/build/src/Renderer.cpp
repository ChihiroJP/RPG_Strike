...
bool Renderer::Init(ID3D11Device* device, ID3D11DeviceContext* context)
{
    ...
    hr = D3DCompile(g_ShaderCode, strlen(g_ShaderCode), NULL, NULL, NULL, "VS", "vs_5_0", 0, 0, &vsBlob, &errorBlob);
    if (FAILED(hr)) {
        if (errorBlob) {
    ...
    hr = D3DCompile(g_ShaderCode, strlen(g_ShaderCode), NULL, NULL, NULL, "PS", "ps_5_0", 0, 0, &psBlob, &errorBlob);
    if (FAILED(hr)) {
        if (errorBlob) {
